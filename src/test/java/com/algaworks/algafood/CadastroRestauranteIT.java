...
import com.algaworks.algafood.util.JSONArrayUtil;
...
public class CadastroRestauranteIT {
	...
	private String jsonRestaurantesInvalidos;
	...
	public void setUp() {
		...
		jsonRestaurantesInvalidos = ResourceUtils.getContentFromResource(
				"/json/incorreto/restaurante-lista.json");
...
//	----------------------------
	@Test
	public void deveRetornarStatus400_E_umObjetoInvalido_QuandoSalvarRestauranteTaxaFreteNegativo() {
		RestAssured.given()
			.body(jsonRestauranteSemFrete)
			.contentType(ContentType.JSON)
			.accept(ContentType.JSON)
	...
	@Test
	public void deveRetornarStatus400_QuandoSalvarRestaurantesInvalidos() {
		
		JSONArrayUtil jsonArrayUtil = new JSONArrayUtil(jsonRestaurantesInvalidos);
		String[] jsonRestaurates = jsonArrayUtil.getJsonObjects();
		IntStream.range(0, jsonRestaurates.length).forEach( i -> {
			String jsonRestaurante = jsonRestaurates[i];
			RestAssured.given()
	...
	private void prepararDados() {
		Cozinha cozinhaTailandesa = criarCozinha("Tailandesa");
		Cozinha cozinhaAmericana = criarCozinha("Americana");
		...
		burgerTopRestaurante = criarRestaurante("Burger Top", "10", cozinhaTailandesa);
		ArrayList<Restaurante> restaurantes = new ArrayList<>();
		restaurantes.add(burgerTopRestaurante);
		restaurantes.add(criarRestaurante("bbb", "9.0", cozinhaAmericana));
		restaurantes.forEach(rest -> restauranteRepository.save(rest));
